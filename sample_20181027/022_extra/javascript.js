...
(() => {
    ...
    function init(texture){
        ...
        scenePrg.uniLocation[2] = gl.getUniformLocation(scenePrg.program, 'moveParam'); // @@@
        ...
        for(let index = 0; index < COUNT; ++index){
            ...
            for(let i = 0; i <= count; ++i){
                ...
                for(let j = 0; j <= count; ++j){
                    ...
                    positionArray[index].push(x, y, 0.0);
                    colorArray[index].push(i / count, j / count, index / COUNT, 1.0);
                }
            }
            // 頂点座標の配列から VBO（Vertex Buffer Object）を生成する
            VBOArray[index] = [
                createVbo(positionArray[index]),
                createVbo(colorArray[index])
            ];
        }
        ...
        function render(){
            ...
            // カメラ関連のパラメータ
            let cameraPosition    = [0.0, 0.0, 0.0];  // カメラの座標 @@@
            let centerPoint       = [0.0, 0.0, -1.0]; // カメラの注視点 @@@
            ...
            for(let index = 0; index < COUNT; ++index){
                // VBO を有効化 @@@
                setAttribute(VBOArray[index], scenePrg.attLocation, scenePrg.attStride, null);

                // モデル座標変換
                let rotationRadian = (360 / COUNT) * index * Math.PI / 180;
                mat.identity(mMatrix);
                mat.rotate(mMatrix, rotationRadian, [0.0, 1.0, 0.0], mMatrix);
                ...
                // index の値に応じて異なる移動係数が送られるように @@@
                let i = (nowTime % COUNT) - index;
                let param = Math.min(Math.max(i, 0.0), 1.0);
                gl[scenePrg.uniType[2]](scenePrg.uniLocation[2], param);

                // 描画
                gl.drawArrays(gl.POINTS, 0, positionArray[index].length / 3);
            }
